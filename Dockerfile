...
RUN set -x \
    ...
    && curl -Ls          "https://product-downloads.atlassian.com/software/jira/downloads/atlassian-jira-software-8.14.1.tar.gz" | tar -xz --directory "${JIRA_INSTALL}" --strip-components=1 --no-same-owner \
    && rm   -rf          "${JIRA_INSTALL}/atlassian-jira-software-8.14.1.tar.gz" \
